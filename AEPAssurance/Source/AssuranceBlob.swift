...
enum AssuranceBlob {
    ...
    static func sendBlob(_ blob: Data, forSession session: AssuranceSession, contentType: String, callback : @escaping (String?) -> Void) {
        guard let sessionDetails = session.sessionDetails else {
            Log.error(label: AssuranceConstants.LOG_TAG, "Assurance Session details are nil. Unable to send blob data.")
            return
        }
        var components = URLComponents()
        components.scheme = HTTPS_SCHEME
        components.host = String.init(format: HOST_FORMAT, sessionDetails.environment.urlFormat)
        components.path = UPLOAD_PATH
        components.queryItems = [
            URLQueryItem(name: QUERY_VALIDATION_SESSION_ID, value: sessionDetails.sessionId)
        ]
